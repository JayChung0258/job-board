...
class TestSearchService:
    ...
    def mock_jobs_data(self):
        ...
        mock_jobs_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "app/data/mock_jobs.json"
        )
    ...
    def test_search_by_query(self, search_service, mock_db, mock_jobs_data):
        ...
        # Mock filtered job
        python_job = [job for job in mock_jobs_data if "Python" in job["job_position"]][
            0
        ]
    ...
    def test_search_by_location(self, search_service, mock_db, mock_jobs_data):
        ...
        # Mock filtered job
        sf_job = [
            job for job in mock_jobs_data if "San Francisco" in job["job_location"]
        ][0]
        mock_job = MagicMock(spec=Job)
    ...
    def test_search_with_pagination(self, search_service, mock_db, mock_jobs_data):
        ...
        assert result["pages"] == (len(mock_jobs_data) + 1) // 2
